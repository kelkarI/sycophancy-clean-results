...
════════════════════════════════════════════════════════════════════════
BACKGROUND CONTEXT — what this project is and why we are doing it
════════════════════════════════════════════════════════════════════════

The project asks whether general-purpose **persona steering vectors**
(`skeptic`, `devils_advocate`, `judge`, etc., extracted from open-ended
role-playing — *not* from sycophancy data) can reduce sycophantic
behaviour in instruction-tuned LLMs as effectively as a **targeted
Contrastive Activation Addition** (CAA, Rimsky et al. 2024) vector
trained specifically on sycophancy A/B contrasts. The headline finding
of the parent paper (§3) is that a `skeptic` persona direction reduces
sycophancy to within ~10% of CAA's effect on both Gemma 2 27B and
Qwen 3 32B — even though the persona vector saw no sycophancy data
during extraction.

**§4.3 of the parent paper** makes a geometric claim: the persona
steering vectors and the CAA vector are nearly orthogonal at the
injection layer — |cos(v_persona, v_CAA)| < 0.17 on Gemma, ≤ 0.108 on
Qwen. The paper infers from this that the two reductions arise via
*different mechanisms*.

**THE FOLLOW-UP THIS PROMPT EXECUTES.** A reviewer pointed out that
orthogonality of *steering vectors* at the injection layer does not
imply *mechanistic* independence — 24 (Gemma) to 32 (Qwen) nonlinear
transformer blocks process the perturbation downstream and can
collapse orthogonal inputs onto shared pathways. The new experiment
captures the post-block residual at every layer ℓ ≥ TARGET_LAYER
under baseline + CAA-steered + persona-steered forward passes,
computes ΔH = H_steer − H_baseline at each layer, and tracks
cos(ΔH^CAA, ΔH^persona) across depth (fig9). It also plots
‖ΔH_ℓ‖ per layer per condition (fig10) to rule out collapse-to-zero,
which would make any high downstream cosine uninformative.

The result either supports §4.3's mechanistic-independence story
(cosines stay near 0 through the stack) or undermines it (cosines
drift toward 1). This experiment is descriptive: we report the
curves and the sanity checks. Mechanistic *interpretation* belongs
in `role-based-steering/paper/`, not in clean-results.

────────────────────────────────────────────────────────────────────────
Reading order BEFORE writing any code
────────────────────────────────────────────────────────────────────────

  1. THIS PROMPT, end to end.
  2. IMPLEMENTATION_SPEC.md (URL at the very top of this prompt) —
     the algorithmic source of truth. Read sections §1 (repo
     structure), §3 (layer indexing convention), §5 (the new
     experiment's procedure), §6 (sanity checks). Sections §2 and
     §4 are also short and worth skimming.
  3. The cross-model paper repo (one-stop overview):
     <https://github.com/kelkarI/role-based-steering>
     — read paper/RESULTS.md for the headline numbers and the §4.3
     claim being tested. paper/METHODS.md documents the steering
     mechanism (ActivationSteering at the post-block residual,
     unit-normed vectors, sign convention).
  4. The clean-results README (the figure / table package the new
     fig9–fig10 will plug into):
     <https://github.com/kelkarI/sycophancy-clean-results>
  5. The Gemma-side pipeline README (Qwen is a thin variant; reading
     this is sufficient for both):
     <https://github.com/kelkarI/sycophancy-gemma>
...
────────────────────────────────────────────────────────────────────────
  ...
  Models under study:
    - google/gemma-2-27b-it    Gemma 2 27B Instruct, layer 22 of 46
        <https://huggingface.co/google/gemma-2-27b-it>  (gated)
    - Qwen/Qwen3-32B           Qwen 3 32B, layer 32 of 64
        <https://huggingface.co/Qwen/Qwen3-32B>          (gated)

  Steering library:
    safety-research/assistant-axis on GitHub
        <https://github.com/safety-research/assistant-axis>
    Provides the ActivationSteering context manager that mutates the
    residual stream at TARGET_LAYER. INV-1 in this prompt pins its
    forward-hook ordering semantics.

  Pre-computed persona vectors:
    lu-christina/assistant-axis-vectors (HF dataset, public)
        <https://huggingface.co/datasets/lu-christina/assistant-axis-vectors>
    Unit vectors for 9 personas at TARGET_LAYER, for both Gemma 2 27B
    and Qwen 3 32B. The Gemma vectors are committed in
    `sycophancy-gemma/vectors/steering/` (.gitignore on Gemma allows
    this directory; only the raw `vectors/gemma-2-27b/` HF mirror
    is gitignored). The Qwen vectors are NOT committed (license);
    regenerated by recipe R1+R3 in the QWEN VECTOR REGENERATION
    section below.

  CAA method:
    Rimsky et al. 2024, "Steering Llama 2 via Contrastive Activation
    Addition" — <https://arxiv.org/abs/2312.06681>
    The CAA vector for each model is extracted locally on that model
    from a 2000-pair sycophancy training set (NLP survey + political
    typology quiz from anthropics/evals). On Gemma the CAA vector is
    committed; on Qwen it is regenerated by recipe R2 below.

  Benchmark:
    Perez et al. 2023, "Discovering Language Model Behaviors with
    Model-Written Evaluations" — <https://arxiv.org/abs/2212.09251>
    We use the `sycophancy_on_philpapers2020.jsonl` subset
    (300 base questions × 2 orderings = 600 counterbalanced rows)
    as the held-out test set on each model. Source:
    <https://github.com/anthropics/evals/tree/main/sycophancy>.

  Public mirror of the general experiment code:
    <https://github.com/vkmk1/Sycophancy-Steering> — link-only;
    DO NOT modify. Phase 6 / Phase 7 ensure this URL lands in the
    clean-results README and in both source-repo "See also" sections.

────────────────────────────────────────────────────────────────────────
Self-check before proceeding to Phase 0
────────────────────────────────────────────────────────────────────────

After reading the prompt + the spec + the parent paper README, you
should be able to explain in your own words, without re-checking:

  (a) What we are measuring (cos(ΔH^CAA_ℓ, ΔH^persona_ℓ) per layer
      per prompt, averaged within prompt over tokens then across
      prompts; plus ‖ΔH_ℓ‖ per layer).
  (b) Why a uniformly high cosine downstream would undermine the
      §4.3 mechanistic-independence claim even though the steering
      vectors are orthogonal at injection.
  (c) Why a uniformly near-zero cosine downstream would support §4.3.
  (d) Why the §6.2 injection-layer sanity check must agree with the
      sign-flipped §4.3 vector cosine numbers (because at the
      injection layer ΔH = α·v exactly, before any nonlinear block
      has acted).
  (e) Why ‖ΔH‖ collapsing to zero downstream would make the cosine
      reading at that layer uninformative (cosine of a zero-ish vector
      with anything is numerically meaningless even if mathematically
      defined via the eps-floor in `per_token_cosine`).

If you can't answer any of (a)–(e) cleanly, re-read the spec and the
parent paper RESULTS.md before continuing.
